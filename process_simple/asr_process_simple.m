...
maxdims = 1;
usegpu = false;
maxmem = 1024; % hlp_memfree/(2^21);
maxdims = round(size(data,1)*maxdims);
...
for i=1:splits
    ...
    if ~isempty(range)
        ...
        for j = 1:length(update_at)
            ...
            [V,D] = eig(Xcov);
            % just in case. The decompsition will return complex values in case of bad (too small,
            ...
            if ~trivial || ~state.last_trivial
                ...
                %data_old(:,subrange) = bsxfun(@times,blend,R*data(:,subrange)) + bsxfun(@times,1-blend,state.last_R*data(:,subrange));
